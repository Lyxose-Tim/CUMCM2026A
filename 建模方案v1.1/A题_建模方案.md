...
| 范围 | Q1–Q4 全部 |
| 状态词约定 | 「已决」= 用户在对话中拍板；「设计已具备」= 规格已写清、尚未执行；「已实测通过」= 当前结构化记录的 metric 满足 limit；「历史探针」= 旧探索性复算；「尚未执行」= 需下游运行；「待人工核验」= 参赛队须核对 |
| 本文档性质 | 方案与实现规格。**候选 PDE 场解、网格/时间加密和候选烘干时长已结构化重跑，事实源为 `reports/verification.json`；D12 尚未授权，正式生产场解、正式 t\* 与 result1–4 仍未执行**。本文候选数值不是正式答案。独立审计结论：主模型设计基本对齐，无需推翻重建；修订重点为收缩范围、修正验收、如实证据，而非新增干燥机理 |
...
| 数值 | 验证基线：N=200，BE Δt=1 s。候选（待 D12）：由全输出对象 V-3 对照选 N（当前流程默认 N=800、以 N=1600 作空间参考），BDF（6.0.4）；BE Δt≤0.25 s 仅作验证对照。单条同源轨迹覆盖 0–1800 s（不拼接精细解前 100 s 到粗网格轨迹）；若采用表层加密非均匀网格，按 6.0.2 规则整体更新几何 |
...
### 8.1 检验清单（验收定义与生产阶段历史状态）

> 本轮候选实际状态以 `reports/verification.json` 为唯一事实源：V-1、V-2、V-3、V-4a、V-4c、V-5、V-6、V-10、V-13 及 D12 所需的 V-11 求积点加密已实测通过；V-4b 的 BE 1 s 粗步失败而 0.25 s 细步通过；V-11 的附加界面时长对照为局部执行；V-7、V-8、V-9、V-12、V-15 未运行。下表末列中的“正式执行尚未”仅指生产配置和正式 `result1–4` 尚未获授权/执行，不表示候选检查未运行。

| 编号 | 检验 | 方法 | 判据 | 生产/历史状态（候选实测见上方事实源） |
|---|---|---|---|---|
...
| V-4c | 有效热方程残差（审计 A-F02：W/m 与 W 口径二择一，全程一致）| **按单位长度定义**（量纲 W/m）：$\mathcal R_E^{(\ell)}=2\pi\displaystyle\int_0^{R(t)}b(C)\,\mathcal D_tT\,r\,dr-2\pi R(t)h\big[T_{air}-T_s\big]$，其中物质导数 $\mathcal D_tT$ 在 Q1–Q3 即 $\partial_tT$、Q4 为仿射物质导数（6.0.3）；时间格式用实际方案（BE 差商；BDF 用求解器一致性）。**参考尺度同取单位长度：$2\pi R(t)h\max(|T_{air}-T_0|_{max},1\ \mathrm K)$（删去 L，消除初温差为零的除零）**。**若导数直接取自同一离散 RHS，则本项只检「所选离散方程的代数平衡」，不能独立证明时间积分准确——时间精度仍由 V-3 独立加密核验**（PATCH-05）。（若改用全长总功率口径，则两项均乘 L、阈值单位改为 W——二者不得混用）| 绝对 ≤1e-6 W/m 且相对 ≤1e-8；$b(C)T_t$ 方程的积分残差 ≠ $\dfrac{d}{dt}\!\int b(C)T\,dV$，**不冒称真实开放系统焓守恒** | 代码 `energy_residual_be`（W/m）已实现，探针 RE≈3.2e-10 W/m；正式执行尚未 |
...
| V-13 | Q4 几何与守恒 | $\bar C$ 收支（含 1/R 因子）；独立核对 $1/R^2$、$1/R$、附件半径与外推标记；E06/E07 诊断图仅为可选说明 | 几何与收支检查通过；诊断图不作必需项、不强制入论文 | 检查器已实现；实际状态以 `reports/verification.json` 为准 |
...
| S9（可选） | Q4 几何诊断 | $R_C(\bar C)$ vs 数据 | Q4 | 可选诊断图 | 0（后处理） | 仅诊断，不驱动模型；不作候选验收或论文必备图 |
...
| 类别 | 内容 |
|---|---|
| 历史探针（探索性；与当前结构化重跑分开） | E01–E16（v1.0）；E17 长时空间/通量；E18 解析解空间/时间；E19 收支与梯形差；E20/E23 早期表面层；E21 事件合成；E22 端面；E24 附录 4 固定半径条件对照 |
| 本轮候选已执行 | N=800 候选、N=1600 空间参考与独立 BDF 时间加密；Q1:V-1/2/3/5/11、Q23:V-3/4/5/6/11、Q4:V-3/4/5/6/10/11/13 的适用门控均通过；Q23 S1–S6 及时间加密已运行；准确状态、最大误差和配置摘要见 `reports/verification.json`、`reports/sensitivity.json` |
| 尚未执行 | D12 生产配置授权；生产续算/导出编排；正式 $t^*_3$、$t^*_4$ 与 result1–4；导出后的 V-8 result2/3 跨文件一致性、V-9 工作簿结构终检；S7/S8/V-14、S9 诊断图为可选/历史，不入必做全项 |
...
| `verify.py` / `writers.py` | `analytic_cylinder_robin`, `hi_precision_reference`（紧容差 BDF，非精确半离散解，审计 A-F06）, `order_from_errors`, `mass_balances_be`（V-4a/b，BE）, **`bdf_mass_balance_record`（V-4a/b，BDF 增广态 I、独立求积与时间加密）**, `energy_residual_be`（W/m 口径，审计 A-F02）, `envelope_record`, `static_limit_q4`, `verify_workbook`, `verify_result23_consistency` 已实现；`end_effect_increment` 待补（V-15 未运行）；`latent_compat_probe`（**可选，已移出必做**）| 报告 JSON/Markdown | 8.1–8.3 |
...
| 项 | 规格 |
|---|---|
| 网格 | 验证基线 N=200；候选 N=400/800/1600；当前可复现流程以 Q1/Q23/Q4 的 N=800 对 N=1600 作全输出对象空间复核；**网格只由该问 V-3 实测误差选定，不设 N≥800 或 Q1≥1600 的无条件门槛**；最终值分问记入 D12（下表）|
...
**生产配置分问记录表（D12；每问各自记录、Q4 单独验收）**——候选实测结果已列出；最终列仍只由用户 D12 授权填写，不由程序自动提升：

| 问 | 候选网格与本轮实测 | 最终网格（待授权）| 积分界面/求积阶数 | BDF 容差 | 步长上限 | 适用检查与本轮状态 |
|---|---|---|---|---|---|---|
| Q1 | N=800 对 N=1600 空间参考及时间加密 | null；待 D12 | `integral` 8 点（16 点全输出对照）| rtol 1e-8，atol_C 1e-11、atol_T 1e-8 K | 空气段 60 s | V-1/V-2/V-3/V-5/V-11 均通过 |
| Q23 | N=800 对 N=1600；候选 $t^*=57.47402726$ h | null；待 D12 | 同上 | 同上，另以 rtol 1e-10 时间加密 | 60 s→600 s，14400 s 重启 | V-3/V-4/V-5/V-6/V-11 均通过 |
| Q4 | N=800 对 N=1600；候选 $t^*=51.09202937$ h | null；待 D12，**不因 Q23 同配置而豁免** | 同上 | 同上，另以 rtol 1e-10 时间加密 | 同上 | V-3/V-4/V-5/V-6/V-10/V-11/V-13 均通过；r=1.2 cm 最差点已登记 |
...
**本轮已落地（PATCH-04 + GitHub 审计 F03）**：`src/drymodel/config_check.py` 是唯一权威实现，方案目录 `config_check.py` 仅作可独立运行的薄入口。两者共用显式 `ConfigError`（不用 `assert`，`-O` 下仍生效），并实测拒绝旧九类和本轮新增的非有限值、布尔/字符串、负步长/容差、非整数迭代数、错误输出规则及伪授权证据。求积点、断点、BDF/BE 设置和情景幅度均进入实际有效配置快照。「配置解析通过 ≠ 模型验证通过」。
...
- 拒绝旧九类及审计新增非法变异（作为负例单元测试）；布尔不冒充整数网格；
- 公式字符串不 `eval`：校验说明式与冻结附录式一致（防串组/笔误）；求解器实际调用的 `D_app2/props_app3/props_app4` 由 `tests/test_props.py` 独立覆盖，不由检查器文字推断；
...
| 2 | **候选计算（目标导向，非全组合竞赛；审计 A §8）**：以 `integral` 界面 + BDF 为主候选，`harmonic`、BE 仅作对照；各问按全输出对象误差逐级加密 N，不以早期粗网格相邻差直接指定最终最低 N；**Q23/Q4 运行至阈值括区间；Q1 仅解到 1800 s（不求 0.15 达标事件）**；**不预先把网格×界面×时间方法全组合跑完**；缓存输出行与标量 | `cache/candidates/*` |
...
| 7 | 灵敏度 S1–S6、**S10（附录 4 固定半径对照，8.7）**（每情景重新积分并用实测加密误差判别）；S9 仅可选诊断，S7/S8 已降级为可选/后处理 | `reports/sensitivity.md` |
| 8 | MATLAB 绘图 | `figs/*` |
...
| 风险 | 表现 | 入口（均为候选，非保证通过） |
|---|---|---|
| 长时空间/通量误差 | 调和平均 N=200→400 的 $t^*$ **相邻差**约 1.2710 h（不是对精细参考的误差） | 优先测试 `integral` 界面；网格逐级对照；时间与空间误差分栏检查 |
...
| 项 | 性质 | 处理 |
|---|---|---|
| D12 生产配置授权 | 待用户确认 | 分问登记经适用 V 项实测通过的具体 N、`integral` 求积点数与 BDF 配置；不预设无条件最低 N；验证记录和配置摘要绑定后回报 |
| 候选数值结果 | 已实测 | 只从 `reports/verification.json` / `reports/sensitivity.json` 引用，不改写为正式答案 |
| 正式数值结果与 result1–4 | 尚未执行 | D12 授权并实现生产编排后生成；不得预填 |
