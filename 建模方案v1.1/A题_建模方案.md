...
| 项目 | 内容 |
|---|---|
| 文档版本 | v1.2（在 v1.1 基础上按《A题_独立审计与修改要求.md》修订：修正审计 A-F01/A-F02/A-F05/A-F06，新增附录 4 固定半径对照，将潜热等从必做运行与主文中移出；配套《A题_决策日志.md》v1.1、`A题_config.yaml`、`config_check.py`、《A题_自查报告.md》） |
| 本次修订范围 | **本次仅修订本方案文档一份**，其余四份配套文件（决策日志、config.yaml、config_check.py、自查报告）按用户要求保持不动。因此独立审计中主要落在这四份文件上的 A-F03（检查器）、A-F04（config 数值键）、A-F07（自查证据状态）只在本方案涉及的文字（D12、§9.2–9.4、§9.6 对检查器/配置/证据的表述）内同步修正为「规格要求 + 如实状态」，四份文件本身的落地留待后续单独修改 |
| F 编号约定 | 本文中**不带前缀**的 F01–F10 指旧《A题建模方案修改要求.md》；**带「审计 A-」前缀**的 F01–F07 指本次《A题_独立审计与修改要求.md》。两套编号互不相同，不可混用 |
| 日期 | 2026-09-11（v1.2 修订同日） |
...
| **D12** | 生产数值配置——**分问记录（审计 A-F04）** | Q1 / Q23 / Q4 **各自**记录候选网格、最终网格、积分界面求积阶数（8/16 点 Gauss 独立键）、BDF 容差、步长上限、已通过的误差检查（见 9.2 分问表）。候选：`integral` 界面 + BDF（rtol 1e-8）；**网格按误差选定，不把 N≥1600 写成无条件物理要求**；Q4 **不因「与 Q2 用同配置」即视为已过自己的精度检查**。BE Δt=1 s、N=200、`harmonic` 仅为验证基线 | **待用户授权**（须先通过 V-1～V-13） |
...
| result2 固定 3 h / 72 h | 历史备选，开关保留但默认不用 |
...
| R05 | 表 1/2 | Q1 | 论文 | 同一未舍入解在 t=100…1800 s、r=0…2 cm 取值 | 6.1, 9.5 |
...
| 模板（**本次已对附件 3 实物核对，2026-09-11；审计 A §2/§7**）| result1/2：工作表「温度」「水分浓度」，A1=`时间\到药材中心的距离`，表头 0, 0.1, 0.2, `…`, 2，A 列 1, 2, 3, `…`；result3/4：Sheet1，A1 同上、表头 0, 0.1, 0.2, `…`, 2、A 列 60, 120, 180, `…`；result4 末表头「药材表面」（其表头为 0, 0.1, 0.2, `…`, 药材表面）。模板均为**含 `…` 占位的缩略示例**（读回确认：`result1-4.xlsx` 首行/首列如上，正式输出须把占位展开为完整列/行）。PDF 可直接确定者：result1/2 工作表名「温度」「水分浓度」、各文件 A 列时间单位 s、第一行距离单位 cm |
...
| H12 | 干物质基准 $\rho_{s0}=\rho(C_0)/(1+C_0)$ 按表达式计算（275.0 / 278.7 / 231.0 为显示近似），适用于 Q1–Q4 的质量换算；替换 $\rho_{s0}$ 仅不影响当前无潜热有效主线的 C/T（审计 A §4.2：故**不必做 $\rho_{s0}$ 正式灵敏度分析**，仅换算，见 S7 降级）；潜热或混合焓拓展中不再只影响换算。附注（审计 A §4.2）：若把附录 4 的 ρ 解释成当前体积湿质量密度、同时要求干物质守恒 + 长度不变 + $C\ge0$，可推出 $R\ge1.2112029565$ cm，而附件 2 终值 1.198 cm——仅说明该**精确假设组合不能同时成立**，不表示实测存在干物质损失，也不授权修改附件或题给系数；故主线保留全部题给 ρ、$c_p$，把 $b=\rho c_p$ 作有效体积热容用，不再额外施加 $R=R(C)$ 结构闭合 | C | Q1–Q4 | 报告量 | 备选 ρ(0) | D06 |
...
| 项 | 内容 |
|---|---|
| 问题分析 | 常物性，D 只依赖 C → 热质解耦；Q1 窗口全在升温段（$T_{air}$ 28→41.5 °C）；$Bi_h$=1.39、$Bi_m$=3.24；$R_0^2/\alpha$≈0.66 h → 30 min 内温度仍在瞬态；早期表面层厚度 $\sqrt{Dt}$≈7e-5 m（1 s）小于 Δr（N=200）→ 早期行需细网格 |
...
| 输出映射 | result1：A 列 1…1800，列 0, 0.1, …, 2.0（节点 $jN/20$）；「温度」（°C）、「水分浓度」；表 1/2：t=100…1800 s，r=0, 0.5, 1, 1.5, 2 cm |
...
| 事件（D11-M/H14，F06 修订） | 在与表格相同的未舍入重构场上求根：BE 每步 $g(a)=\max_i[(1-a)C_i^n+aC_i^{n+1}]-0.15$，$a\in[0,1]$，g 为凸的分段线性函数，用括区间求根（brentq）得 $t_{cross}=t_n+a^*\Delta t$（**不是**先取两端最大值再插值——合成回归：两节点 (0.151,0.1505)→(0.148,0.1495) 的正确根为 0.5 s，端点极值插值给 2/3 s）；BDF 用同一连续解（稠密输出）对 $\max_iC_i(t)-0.15$ 求根。不回穿依据：无源主线在 $D>0$、$R>0$、$h_m>0$、全部已授权 $C_{env}<0.15$ 及比较原理适用下，常数 0.15 是上解 → $t^*=t_{cross}$；续算 600 s 只作数值异常检查（根上等号不是回穿；根后检查用容差 $C_{max}\le0.15+10^{-9}$）。$t_{sample,3}$：首个 60 s 网格点使实测（未舍入）$C_{max}<0.15$。$t_{safe,3}$：首个时刻使 $C_{max}<0.15-\delta$（数值裕量，非真实安全保证）。Δt*、δ 为经验误差估计，非认证误差界 |
| 调度（F06/F08） | ① 候选运行获得阈值括区间；② 网格/时间加密（V-3、V-11）；③ 选定通过验收的生产配置（D12 记录）；④ 据该配置与更精细对照估计 δ、Δt*；⑤ 用生产配置实际续算至 $\max\{t_{end,1s},t_{sample,3},t_{safe,3}\}$ 并再续 600 s；⑥ 统一导出。**首次调用不预设尚未可得的 $t_{final}$；生产配置变化后旧误差标签作废，全部 result/表/图重生成；不以积分区间外稠密输出冒充续算** |
| 输出映射 | result2：A 列 1…$t_{end,1s}$（首个整数秒 $t$ 使未舍入 $C_{max}(t)<0.15$，由连续解在整数秒求值），两工作表；result3：A 列 60, …, $t_{sample,3}$；表 3/4：t=0.5…3.0 h；表 5：t=6, 12, …（≤$t^*$ 的整 6 h，若 $t^*$ 恰为整 6 h 则不重复）+ 末行「烘干结束时间」= $t^*$（h，4 位小数；注明为阈值穿越时刻且等于严格达标集合下确界），值取 $t^*$ 时刻的同一未舍入重构（不预填）；脚注 $t_{sample,3}$、$t_{safe,3}$、±Δt* |
| 检验点 | V-3（含 48 h 等长时表点、表面通量、$\bar C$、$t^*$）、V-4（两项）、V-5、V-6、V-7、V-8、V-9、V-11、V-12 |
...
| 数值 | 6.0.3 + 6.0.4；界面系数与网格同 Q2/Q3 的生产配置（D12）；事件、调度同 6.2（在 x 网格节点上取 $c_{max}$）；$t_{final,4}$ 按 6.2 调度确定 |
| 输出映射（D09/D11-O） | result4：A 列 60, …, $t_{sample,4}$；列 0, 0.1, …, 1.9 cm（20 列）+「药材表面」；每行 $x_j=r_j/R(t)$（未舍入插值 R）；域内判定与掩码共用同一函数 `inside(r_j,R,tol)`：$r_j\le R(1+10^{-12})$；$x_j<1$：节点线性插值；$x_j=1$（容差内）：取 $c_N$，与「药材表面」列同一值；域外：单元格留空（None）；域内重构失败 → 抛错。各列最后有效时刻（线性插值 R，含相等）：1.9 cm 0.3937 h；1.8 cm 0.962 h；1.5 cm 3.6515 h；1.4 cm 5.444 h；1.3 cm 8.4583 h；1.2 cm 57.5 h；≤1.1 cm 全程。表 6：列 0, 0.5, 1.0, 药材表面（1.5、2.0 在 t≥6 h 全部域外，正文说明），行 6, 12, …（不与末行重复）+ 末行 $t^*_4$；配套小表：各行 R(t)（及是否外推）；T 不写入 result4 |
| 检验点 | V-3（含晚期剖面）、V-4（两项，含 1/R 因子）、V-5、V-6、V-9（掩码 ⇔ 几何）、V-10（静态极限：R≡$R_0$、附录 4 物性、同一初边值，动域求解器 vs 固定域求解器逐点比较，不要求附录 4 结果等于附录 3）、V-13、V-12 |
...
| **D12（待确认）** | 生产数值配置——**分问记录**（界面系数、网格、时间格式；审计 A-F04）| 候选：`integral` 界面 + BDF（rtol 1e-8）；Q1/Q23/Q4 各自按误差选定网格并单独验收（9.2 分问表）；**N 按误差选、非无条件物理要求；Q4 不因与 Q23 同配置而豁免**；BE 保留为验证基线 | **未确认** | E17–E20 探针（探索性）；须经 V-1～V-13 验收后授权 | 【AI 提议】 |
...
| V-4a | 离散代数收支 | 用实际时间格式的通量权重：BE 满足 $\bar C_N-\bar C_0=-\Delta t\sum_{n=1}^Nf_n$（$f_n$ 取步末）；BDF 不检验 BE 差商残差，改用累计通量作辅助 ODE 状态 $\dot I=f$ 与 $\bar C$ 的一致性 | 相对残差 ≤1e-12（BE，探针 3.6e-14/6.3e-14）；BDF ≤10×rtol | 探针已复现（E19）；正式执行尚未 |
...
| V-11 | 界面通量一致性（B11，扩展到 Q1–Q4） | `harmonic` vs `integral`（8 点，16 点对照）在 N=200/400/800 下比较：表点、Q2/Q3 与 Q4 晚期剖面、表面通量、$\bar C(t)$、$t^*$ | 候选须同时通过 V-1～V-10、V-13；两法差随 N 收敛；生产采用何种界面由 D12 记录授权 | 探针已复现（E17）；正式执行尚未 |
...
| 尚未执行 | 全部正式 PDE 场解；V-1～V-15 正式执行；灵敏度 S1–S7、S9、S10（附录 4 固定半径对照）；$t^*_3$、$t^*_4$ 正式值；result1–4 生成；D12 生产配置授权 |
...
| `operators.py` | `flux_C`, `flux_T`, `rhs(t,y)`, `residual_rate(t,y,ydot)`, `jac_sparsity(N)` | 状态排列 `y=[C_0..C_N, T_0..T_N]`；2×2 块三对角 | 残差为速率形式（单位 kg/kg/s、K/s） |
| `solver_be.py` | `step_be`, `integrate_be(y0,t_end,dt,breakpoints,callbacks)` | 三对角 `solve_banded`；Picard；减步与再对齐 | 验证基线 |
| `solver_bdf.py` | `integrate_bdf(y0,t_span,cfg,breakpoints,events,sample_times)` | `solve_ivp(BDF, jac_sparsity, dense_output)` 分段重启；采样时刻由稠密输出求值；稠密输出精度另以更严格容差/更小 max_step 独立重算核验（审计 A-F01；`t_eval` 仅保存时刻，不作精度证据） | 生产候选 |
| `criterion.py` | `event_root_reconstructed(t_n,C_n,t_np1,C_np1,thr)`；`event_root_dense(sol,thr)`；`post_check(...)`；`detect(...)->{t_cross,t_star,t_sample,t_safe,argmax_history,delta,dt_star}` | — | F06 规则 |
...
| `verify.py` | `analytic_cylinder_robin`, `hi_precision_reference`（紧容差 BDF，非精确半离散解，审计 A-F06）, `convergence_study`, `balances_be`, `balances_flux_integral`, `energy_residual`（W/m 口径，审计 A-F02）, `envelope_check`, `cross_file_check`, `static_limit_q4`, `end_effect_increment`；`latent_compat_probe`（**可选，已移出必做**）| 报告 JSON/Markdown | 8.1–8.3 |
...
| 状态排列与雅可比 | `y=[C_0..C_N, T_0..T_N]`；稀疏模式：每个块三对角 + 交叉块三对角（T→D，C→b/k） |
...
| 问 | 候选网格 | 最终网格 | 积分界面/求积阶数 | BDF 容差 | 步长上限 | 已通过误差检查 |
|---|---|---|---|---|---|---|
...
集中配置以独立文件 **`A题_config.yaml`**（可由 PyYAML `safe_load` 加载）交付，并附 **`config_check.py`**。要点：**物理常数**集中带 `{value, unit, source}`；数值参数可用简洁字段（不再笼统声称「每个数值参数均带三标签」）；公式字符串仅作说明，运行时映射到 `props.py` 具名函数（**不使用 `eval`**）；`production.approved=false`，生产配置须经验收后写入 `config_id`。

**如实状态（审计 A-F03）**：当前检查器对原配置可解析通过，但**并不证明公式与配置一致**——独立审计对内存副本做的八类非法变异（Q1 密度改 −820、Q2/Q3 的 D 前因子改 2.4e3、Q4 的 D 改 0、`N_default` 改 −200、BDF rtol 改负、result2 终点改 3 h、`energy_form` 改 `enthalpy_advective`、`q4.axial` 改 `shrinking_L`）在原检查器下**仍全部通过**。故「配置解析通过 ≠ 模型验证通过」，更 ≠ 公式—配置一致。

**检查器须达到的规格（下游对 `config_check.py` 的修改要求；本次按用户范围未改动该文件，留待后续）**：

- 用**显式异常或配置 schema**，不只用 `assert`（Python `-O` 优化模式会移除 `assert`，故 `-O` 下不得失效）；
- 校验全部**物理常数与公式编号**、数值**有穷性**、**正范围**、**整数网格数**、**每问物性组**、**固定长度主线**、**事件类型**、**正式输出终点与单位**；
- 拒绝上述**八类非法变异**（作为负例单元测试）；
- 物性函数用明确实现 + 单元测试，在若干正 $C$ 与 K 温度上核对 PDF 代入值；不用 `eval`；
- **主线题给值（h、$h_m$ 等）保持不变**；灵敏度用**单独的 scenario 倍率层**，避免「一边要求基础配置精确等于题给值，一边把改写后的同一基础配置送入同一检查器」的自相矛盾。
...
| 2 | **候选计算（目标导向，非全组合竞赛；审计 A §8）**：以 `integral` 界面 + BDF 为主候选，`harmonic`、BE 仅作对照；各问按误差逐级加密 N（Q1 早期到 ≥1600），运行至阈值括区间；**不预先把网格×界面×时间方法全组合跑完**；缓存输出行与标量 | `cache/candidates/*` |
| 3 | 验证：V-3～V-6、V-10、V-11、V-13；确定生产配置 → 记录 D12（待用户授权） | `reports/verification.md` |
| 4 | 用生产配置**实际续算**至严格 1 s/60 s 采样与裕量时刻，再续 600 s；估计 δ、Δt* | `cache/production/*` |
...
配置变化 → 步 3–9 全部重做，相关 result、表、图统一重生成；候选结果不得混入正式结果。
...
| 编号 | 检查点 | 通过条件 |
|---|---|---|
| A-1 | 配置一致性 | 修订后的 `config_check.py`（9.3 规格）以显式异常/schema 校验通过、**且拒绝八类非法变异**、props 单元测试通过、`-O` 模式不失效；配置与第 0/4/6/9 节逐项一致。（审计 A-F03：解析通过 ≠ 公式—配置一致；检查器文件本次未改动，此为待落地规格）|
...
| 早期表面层 | 1 s 行表面差 N=200→400 达 4e-3 | 均匀 N≥1600、表层加密（整体几何更新）或首段细步；同源轨迹，不拼接 |
...
| 模型评价与推广 | 8.7 附录 4 固定半径对照（分离物性与几何效应）；0.3 未实现项作为改进方向；D02-C 门槛；Ⅰ-B；端面；潜热一句局限（8.3，照录） |
