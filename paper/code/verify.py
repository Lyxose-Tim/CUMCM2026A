...
# --------------------------------------------------------------------------
def eigenvalues_slab(Bi: float, nterms: int = 60) -> np.ndarray:
    """平板对称 Robin 特征值：λ sinλ = Bi cosλ 的前 nterms 个正根。"""
    def f(lam):
        return lam * np.sin(lam) - Bi * np.cos(lam)
    xs = np.arange(1e-6, nterms * np.pi + 5.0, 0.002)
    fx = f(xs)
    roots = []
    idx = np.where(np.diff(np.sign(fx)) != 0)[0]
    for i in idx:
        try:
            roots.append(brentq(f, xs[i], xs[i + 1], xtol=1e-13, rtol=1e-14, maxiter=200))
        except Exception:
            pass
        if len(roots) >= nterms:
            break
...
def analytic_slab_center(Fo, Bi: float, nterms: int = 60) -> float:
    """平板中心（x=0）无量纲 θ：Σ c_n exp(-λ_n² Fo)，c_n=2 sinλ_n/(λ_n+sinλ_n cosλ_n)。"""
    lam = eigenvalues_slab(Bi, nterms)
    cn = 2.0 * np.sin(lam) / (lam + np.sin(lam) * np.cos(lam))
