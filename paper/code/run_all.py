...
def write_status(cfg, data, sens):
    ...
    L = ["# 状态汇总（status.md）\n",
         ...
         f"| 端面校核 V-15 | {'已实测通过' if (REPORTS / 'V15_end_effect.md').exists() else '仍未执行'} | reports/V15_end_effect.md |",
         "| 论文数值/文本、AI 使用详情 | 待人工核验 | — |",
         ("\n## 正式答案（已授权生产、官方结果）\n" if (approved and files_exist)
          else "\n## 候选数值（探索性，非正式答案）\n"),
         "| 量 | 值 |", "|---|---|",
         f"| Q2/Q3 达标时长 t* | {d['q23']['t_star_h']:.4f} h（N={d['q23']['N']}, {d['q23']['interface']}） |",
         f"| Q4 达标时长 t* | {d['q4']['t_star_h']:.4f} h（N={d['q4']['N']}, {d['q4']['interface']}） |",
         f"| S10 ②附录4/R0 t* | {d['s10']['case2_app4_R0'][max(d['s10']['case2_app4_R0'])]:.4f} h |",
         ("\n> 上表为已授权生产配置下的正式结果，与 outputs/result1–4、论文表 1–6 同源。"
          if (approved and files_exist)
