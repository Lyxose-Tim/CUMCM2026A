...
def run_d12_evidence(cfg):
    ...
    # 覆盖至 result3 末行采样时刻（t_sample,3），并显式并入事件行 t*（表5末行）
    t_end1s = int(det23.t_sample)                      # = 206940 s（result3 末行）
    ts_q23 = np.arange(1, t_end1s + 1, dtype=float)    # result2/3 全逐秒行
    ts_q23 = np.unique(np.append(ts_q23, float(det23.t_cross)))   # + 事件行 t*_3
    ts_q4 = np.arange(60, int(det4.t_sample) + 1, 60, dtype=float)  # result4 全分钟行
    ts_q4 = np.unique(np.append(ts_q4, float(det4.t_cross)))       # + 事件行 t*_4
...
def _write_evidence(cfg, r):
    ...
    L = ["# D12 可追溯证据（本轮代码实测复跑）\n",
         ...
         "\n> 注：Q2/Q3 采样轴已延伸至 result3 末行 t_sample,3=206940 s，并显式并入事件行 t*_3；"
         "Q4 采样轴延伸至 t_sample,4 并并入 t*_4。表 5/6 末行（事件行 t*）与末行采样时刻均在覆盖内。\n",
