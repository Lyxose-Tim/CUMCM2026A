...
def _write_json_receipt(cfg, res, outdir):
    ...
    # 结果与终检明细：只要 res 中存在即记录（成功与失败回执均含），失败时并给出失败阶段/项
    q23, q4 = res.get("q23"), res.get("q4")
    if isinstance(q23, dict) and "t_star_s" in q23 and isinstance(q4, dict):
        rec["results"] = {
            "q23": {"t_star_s": q23["t_star_s"], "t_end_1s": q23.get("t_end_1s"),
                    "t_sample_s": q23["t_sample_s"], "post_max_cmax": q23["post_max_cmax"],
                    "cmax_at_tsample": q23["cmax_at_tsample"]},
            "q4": {"t_star_s": q4["t_star_s"], "t_sample_s": q4["t_sample_s"],
                   "post_max_cmax": q4["post_max_cmax"],
                   "cmax_at_tsample": q4["cmax_at_tsample"]},
        }
    if res.get("file_sha256"):
        rec["file_sha256"] = res["file_sha256"]
    if res.get("V9"):
        rec["V9"] = {k: {"ok": v["ok"], "issues": v["issues"][:5]} for k, v in res["V9"].items()}
        failed = [k for k, v in res["V9"].items() if not v["ok"]]
        if failed:
            rec["failed_checks"] = failed
    if res.get("V8") is not None:
        rec["V8"] = {"ok": res.get("v8_ok"), "n_common_times": res["V8"]["n_common_times"],
                     "n_mismatch": res["V8"]["n_mismatch"],
                     "n_missing_coverage": res["V8"].get("n_missing_coverage", 0)}
    if not res["ok"]:
        rec["failed_stage"] = res.get("reason", "unknown")
